...
const CONFIG = {
  ...
  contactEmail: 'slawomir.rurak@onet.pl'
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- FORMULARZ KONTAKTOWY ----------
     Strona jest statyczna (bez backendu), więc formularz składa
     gotową wiadomość i otwiera program pocztowy. Jeśli w przyszłości
     pojawi się backend albo usługa typu Formspree — podmień blok poniżej.
  ------------------------------------------------ */
  ...
  form.addEventListener('submit', e => {
    ...
    let ok = true;
    [['name', data.name], ['phone', data.phone], ['message', data.message]].forEach(([key, val]) => {
      const field = el[key].closest('.field');
      const bad = val.length < 2;
      field.classList.toggle('is-invalid', bad);
      if (bad) ok = false;
    });

    if (!ok) {
      note.textContent = 'Uzupełnij imię, telefon i treść wiadomości.';
      note.className = 'form__note is-err';
      return;
    }

    const subject = `Zapytanie ze strony — ${data.name}`;
    const body =
      `Imię: ${data.name}\n` +
      `Telefon: ${data.phone}\n` +
      `E-mail: ${data.email || '—'}\n\n` +
    ...
    note.textContent = 'Otwieramy Twój program pocztowy z gotową wiadomością. Wolisz szybciej? Zadzwoń: 781 071 789.';
